...
public class Product2Controller : Controller
{
    ...
    public IActionResult Details(Guid? id)
    {
        ...
        var model = _mapper.Map<Product, ProductDetailsModel>(productEntity);
        if (productEntity.Images.Count > 0)
            {
                // get first image
                model.MainImage = productEntity.Images
                    .OrderBy(x => x.SortOrder)
                    .ThenBy(x => x.Position)
                    .FirstOrDefault()
                    .Image
    ...
    public IActionResult Create()
    {
        ...
        {
            CategorySelectList = _viewHelper.GetCategorySelectList(),
            ManufacturerSelectList = _viewHelper.GetManufacturerSelectList(),
            SpecificationKeySelectList = _viewHelper.GetSpecificationKeySelectList()
        };
        return View(model);
    }
    ...
    public IActionResult Create(ProductCreateOrUpdateModel model, bool continueEditing)
    {
        ...
        {
            // check if name exist
            if (_dataHelper.CheckForDuplicate(ServiceType.Product, Helpers.DataType.Name, model.Name))
            {
            ...
            else
            {
                ...
                if (_dataHelper.CheckForDuplicate(ServiceType.Product, Helpers.DataType.Seo, model.SeoUrl))
                {
    ...
    private void SaveImageMappings(ProductCreateOrUpdateModel model)
    {
        ...
        if (model.ImageIds != null)
        {
            for (int i = 0; i < model.ImageIds.Count; i++)
            {
                ...
                if (Guid.TryParse(model.ImageIds[i], out _))
                {
                    // create mapping entity
    ...
    private void SaveManufacturerMappings(ProductCreateOrUpdateModel model)
    {
        ...
        if (model.ManufacturerIds != null)
        {
            foreach (var id in model.ManufacturerIds)
            {
                ...
                if (Guid.TryParse(id, out Guid manufactureId))
                {
    ...
    private void SaveSpecificationMappings(ProductCreateOrUpdateModel model)
    {
        ...
        if (model.Specifications != null)
        {
            ...
            foreach (var spec in model.Specifications)
            {
                ...
                if (Guid.TryParse(spec.Key, out Guid specificationId))
                {
